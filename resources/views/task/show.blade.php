                    ...
                    <div>
                        @if (Auth::user()->rol == 'comprador' || Auth::user()->rol == 'coordinador')
                            <a href="#" type="button" class="btn btn-sm btn-outline-primary btn-round"
                                data-id_tarea={{ $tarea->id }} data-toggle="modal" data-target="#abrirmodalEditar">
                                Agregar Empresa
                            </a>
                        @endif
                ...
                <div class="col-md-12">
                    <div class="row">
                        <div class="col-md-12">
                            ...
                            <div class="tarea-detalles" data-circulo data-fecha-fin="{{$tarea->fecha_fin}}" data-created-at="{{$tarea->created_at}}">
                                <span class="detalle"><strong>Nombre de Tarea :</strong> {{ $tarea->nombre }}</span>
                                <span class="detalle"><strong>Fecha de Finalizacion :</strong> {{ date('d-M-Y', strtotime($tarea->fecha_fin)) }}</span>
                                <span class="detalle"><strong>Días Totales :</strong> {{ $dias_totales }} Días</span>
                                <span class="detalle"><strong>Días Restante :</strong> {{ $fecha_restantes }} Días restante</span>
                                <span class="detalle"><strong>Finalizacion :</strong> {{ $porcentaje }} %</span>
                            </div>
                    ...
                    @foreach ($noAprovado as $proveedor)
                        ...
                        @component('componentes.cardGeneral')
                            @slot('titulo')
                                ...
                                <div class="d-flex">
                                    @if (Auth::user()->rol == 'coordinador' || Auth::user()->rol == 'comprador')
                                        <form action="{{ route('negociaciones.update', ['negociar' => $proveedor->id]) }}"
                                            method="post">
                                            @csrf
                                            @method('PUT')
                                    ...
                                    @if (Auth::user()->rol == 'comprador' || Auth::user()->rol == 'coordinador')
                                        <a href="#" type="button" class="btn btn-sm btn-outline-primary btn-round"
                                            data-id_tarea="{{ $proveedor->id }}" data-tarea="{{ $proveedor->nombre }}"
                                            data-pais="{{ $proveedor->pais }}" data-ciudad="{{ $proveedor->ciudad }}"
                                            data-distrito="{{ $proveedor->distrito }}"
                                            data-direccion="{{ $proveedor->direccion }}"
                                            data-contactos="{{ $proveedor->contacto }}"
                                            data-telefonos="{{ $proveedor->telefono }}" data-email="{{ $proveedor->email }}"
                                            data-descripcion="{{ $proveedor->descripcion }}" data-toggle="modal"
                            ...
                            @endslot
                            @slot('bodyCard')
                                <h6 class="font-weight-bold">Pais: {{ $proveedor->pais }}, Ciudad: {{ $proveedor->ciudad }},
                                    Distrito: {{ $proveedor->distrito }} </h6>
                                <div>
                                    <p>
